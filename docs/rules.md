...
When a system function is documented as two pages, one per form, each page reaches the other through an organic link on the form itself ("the [dyadic form](x-dyadic.md) closely emulates APL2"), not through a trailing "for the other form, see ...". The disambiguation page opens with a single sentence on what the function does, common to both forms, before naming either. Each form then gets an H2 that is the title of its page, linked to it, so that the page's own table of contents lists the forms by name rather than by valence; under each, one sentence introducing that form ends with a colon and is followed by a short example.
